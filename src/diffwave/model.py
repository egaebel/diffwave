...
class DiffusionEmbedding(nn.Module):
    def __init__(self, max_steps):
        super().__init__()
        """
        torch_version_float = float(".".join(torch.__version__.split(".")[:2]))
        if torch_version_float >= 1.6:
            print(f"Torch Version: '{torch_version_float}'", flush=True)
            self.register_buffer(
                "embedding", self._build_embedding(max_steps), persistent=False
            )
        else:
          self.register_buffer("embedding", self._build_embedding(max_steps))
        """
        self.register_buffer(
            "embedding", self._build_embedding(max_steps), persistent=False
        )
